...
| Package | Purpose |
|---|---|
| `@capacitor/core` | Native bridge runtime |
| `@capacitor/android` | Android platform target |
| `@capacitor/ios` | iOS platform target |
| `@capacitor/app` | App lifecycle events (background/foreground/back button) |
| `@capacitor/splash-screen` | Splash screen control |
| `@capacitor/status-bar` | Status bar colour / style |

---

## Troubleshooting

**App shows the offline fallback page**  
→ The server URL in `server.config.json` is unreachable from the device. Check the IP, port, and that the device is on the same network.

**Audio stops when screen turns off**  
→ Add the foreground service permissions to `AndroidManifest.xml` (see above).

**`cap sync` fails with TypeScript errors**  
→ Run `npm install` first — the Capacitor CLI needs its TypeScript types installed.

**Chrome devtools not connecting**  
→ Ensure `webContentsDebuggingEnabled: true` in `capacitor.config.ts`, then visit `chrome://inspect` on your desktop Chrome.
