...
const api = {
  ...
  async getPushConfig() {
    return this._fetch('/api/push/config');
  },

  async registerPushDevice(guid, token, clientId = '') {
    return this._fetch(`/api/users/${guid}/push/register`, {
      method: 'POST',
      body: JSON.stringify({ token, clientId }),
    });
  },

  async sendPushCommand(guid, command, data = {}) {
    return this._fetch(`/api/users/${guid}/push/command`, {
      method: 'POST',
